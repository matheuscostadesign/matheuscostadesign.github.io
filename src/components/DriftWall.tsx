...
const DriftWall = ({
  ...
  const renderTile = (item: DriftWallItem, id: string) => {
    ...
    if (item.href) {
      return (
        <a key={id} href={item.href} target="_blank" rel="noreferrer noopener" className="drift-wall__tile">
          {inner}
        </a>
      );
    }
    return (
      <div key={id} tabIndex={0} role="button" aria-label={item.title ?? 'tile'} className="drift-wall__tile">
        {inner}
      </div>
    );
  };
